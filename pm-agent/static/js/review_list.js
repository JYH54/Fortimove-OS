...
function renderReviewList() {
    ...
    tbody.innerHTML = sorted.map(review => {
        ...
        // Export ready
        const exportReady = (status === 'approved_for_export' || status === 'approved_for_upload') ? '✅' : '❌';

        // Status badge
        const statusBadge = `<span class="badge status-${status}">${status}</span>`;
        ...
        return `
            <tr>
                ...
                <td>${statusBadge}</td>
                <td class="text-center">${hasPrimaryImage}</td>
                <td class="text-center">${exportReady}</td>
                <td><small>${updated}</small></td>
                <td>
                    <a href="/review/detail/${reviewId}" class="btn btn-sm btn-primary">Review</a>
                </td>
            </tr>
        `;
    }).join('');

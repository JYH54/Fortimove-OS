...
class HandoffService:
    ...
    def send_slack_summary(self, items: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"Sending Slack webhook (attempt {attempt+1}/{max_retries})")
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"Slack sending failed after {max_retries} attempts: {e}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"Email sending failed after {max_retries} attempts: {e}")
                    return {"status": "failed", "error": str(e), "attempts": max_retries}

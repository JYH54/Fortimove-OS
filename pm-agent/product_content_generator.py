...
class ProductContentGenerator:
    ...
    def generate_image_design_guide(self, review_data: dict, summary: dict) -> dict:
        ...
        return {
            ...
            "tone_manner": self._generate_tone_manner(category, review_data.get('_content_tone')),
    ...
    def _generate_tone_manner(self, category: str, tone: str = None) -> str:
        """톤앤매너 — tone 파라미터 우선, 없으면 카테고리 기반"""
        tone_map = {
            "premium": "고급스럽고 세련된 톤\n프리미엄 브랜드 느낌\n화이트/골드/블랙 베이스\n최소한의 텍스트, 여백 활용",
            "health": "깔끔하고 신뢰감 있는 톤\n파스텔 또는 화이트 베이스\n과하지 않은 고급스러움\n건강과 웰빙을 연상시키는 색감",
            "trendy": "밝고 에너지 넘치는 톤\nSNS 친화적 비주얼\n트렌디한 타이포그래피\nMZ세대 감성 컬러",
            "value": "실용적이고 정보 중심 톤\n가격 대비 가치 강조\n비교 차트/숫자 활용\n신뢰할 수 있는 데이터 기반",
            "minimal": "간결하고 깔끔한 톤\n최소한의 디자인 요소\n화이트 스페이스 극대화\n핵심 메시지만 전달",
        }
        ...
        if category == "wellness":
            return tone_map["health"]
        return tone_map.get("premium", "모던하고 깔끔한 디자인")
    ...
    def _analyze_price_positioning(self, price, category: str) -> str:
        """가격 포지셔닝 분석"""
        price = price or 0
        if price < 30000:

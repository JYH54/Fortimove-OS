...
# CORS 설정 (settings.ALLOWED_ORIGINS에서 허용 도메인 관리)
app.add_middleware(
    CORSMiddleware,
...
async def readiness_check():
    """Readiness probe — 모든 서비스가 초기화되었는지 확인"""
    services = {
        "ocr": ocr_service is not None,
        "translation": translation_service is not None,
        "risk_detection": risk_detection_service is not None,
        "image_processing": image_processing_service is not None,
        "seo": seo_service is not None,
    }
    ...
    return {
        ...
        "services": {k: "ready" if v else "not_initialized" for k, v in services.items()}
    }
